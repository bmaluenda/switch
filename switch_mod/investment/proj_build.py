...
def define_components(mod):
    ...
    CONSTRAINED_PROJ_BUILDYEARS is a subset of PROJECT_BUILDYEARS that
    only includes existing or planned projects that are not subject to
    optimization.

    proj_predetermined_cap[(proj, build_year) in CONSTRAINED_PROJ_BUILDYEARS] is
    ...
    mod.CONSTRAINED_PROJ_BUILDYEARS = Set(
        dimen=2)
    mod.PROJECT_BUILDYEARS = Set(
        dimen=2,
        validate=lambda m, proj, bld_yr: (
            (proj, bld_yr) in m.CONSTRAINED_PROJ_BUILDYEARS or
            (proj, bld_yr) in m.PROJECTS * m.PERIODS))
    mod.NEW_PROJ_BUILDYEARS = Set(
        dimen=2,
        initialize=lambda m: m.PROJECT_BUILDYEARS - m.CONSTRAINED_PROJ_BUILDYEARS)
    mod.proj_predetermined_cap = Param(
        mod.CONSTRAINED_PROJ_BUILDYEARS,
    ...
    def bounds_BuildProj(model, proj, bld_yr):
        if((proj, bld_yr) in model.CONSTRAINED_PROJ_BUILDYEARS):
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        optional=True,
        filename=os.path.join(inputs_dir, 'proj_build_constraints.tab'),
        auto_select=True,
        index=mod.CONSTRAINED_PROJ_BUILDYEARS,
